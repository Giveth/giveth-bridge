    ...
    if (Date.now() > ethGasStationLastChecked + FIVE_MINUTES) {
        try {
            ...
            if (resp.ok) {
                ...
                ethGasStationLastPrice = web3.default.utils.toWei(`${average / 10 + offsetGwei}`,
                    'gwei'); // response in gwei * 10
    ...
    if (Date.now() > gasNowLastChecked + FIFTEEN_SECONDS) {
        try {
            ...
            if (resp.ok) {
                ...
                if (code === 200) {
                    const { standard } = data;
                    gasNowLastPrice = web3
                        .default.utils.toBN(standard) // response in wei
                        .add(web3.default.utils.toBN(web3.default.utils.toWei(String(offsetGwei), 'gwei')))
                        .toString(10);
